...
contract LibTransferRequest {
    ...
    string internal constant EIP712_DOMAIN_NAME = "Dola Core";

    string internal constant EIP712_DOMAIN_VERSION = "1";

    bytes32 public constant EIP712_DOMAIN_SEPARATOR_SCHEMA_HASH = keccak256(abi.encodePacked("EIP712Domain(", "string name,", "string version,", "address verifyingContract", ")"));

    bytes32 public EIP712_DOMAIN_HASH;

    bytes32 internal constant EIP712_TRANSFER_REQUEST_TYPE_HASH = keccak256(abi.encodePacked("TransferRequest(", "address senderAddress,", "address receiverAddress,", "uint256 value,", "address relayerAddress,", "uint256 relayerFee,", "uint256 salt,", ")"));
    ...
    constructor() public    {
        EIP712_DOMAIN_HASH = keccak256(abi.encode(EIP712_DOMAIN_SEPARATOR_SCHEMA_HASH, keccak256(bytes(EIP712_DOMAIN_NAME)), keccak256(bytes(EIP712_DOMAIN_VERSION)), address(this)));
    }
...
library LibBytes {
    ...
    function sliceDestructive(bytes memory b, uint32  from, uint256  to) internal pure returns ( bytes memory result)    {
        require((from <= to), "FROM_LESS_THAN_TO_REQUIRED");
        require((to < b.length), "TO_LESS_THAN_LENGTH_REQUIRED");
        assembly {        
        result := add(b, from)
    ...
    function readAddress(bytes memory b, uint256  index) internal pure returns ( address  result)    {
        require((b.length >= (index + 20)), "GREATER_OR_EQUAL_TO_20_LENGTH_REQUIRED");
        index += 20;
        assembly {        
    ...
    function writeBytesWithLength(bytes memory b, uint256  index, bytes memory input) internal pure    {
        require((b.length >= ((index + 32) + input.length)), "GREATER_OR_EQUAL_TO_NESTED_BYTES_LENGTH_REQUIRED");
