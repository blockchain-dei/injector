from serializer import readastcompact, writeastcompact, iscompilable, getContractSourceCode, convert_ast_source, writesourcecode
import os
import argparse
import copy
import re
...
def getparent(root, node):
    return getparentbyid(root, node['id'])

def getparentbyid(root, id):
    if 'children' in root:
        for c in root['children']:
            if c['id'] == id: return root
        for c in root['children']:
            p = getparentbyid(c, id)
