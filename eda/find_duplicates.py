...
# CLI INTERFACE #
#################
parser = argparse.ArgumentParser(description='Finds duplicate file content in the given directory')
...
csv_content = 'contract_name;ID_duplicate_contract\n'
duplicates = 0
...
for value in content_file_map.values():
    ...
    for i in range(len(value)):
        # the first is not a duplicate, only the others
        csv_content += f'{os.path.splitext(value[i])[0]};{str(i != 0)}\n'
