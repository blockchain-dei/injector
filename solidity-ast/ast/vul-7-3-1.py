...
Assumptions: not all the tipe can works because they are explicited used with their original type
'''
    ...
    def traverse(node):
        if isinstance(node, dict):
            if node.get('nodeType') in ['VariableDeclaration', 'ParameterList']:
                if node.get('typeName') and 'uint256' in node['typeName'].get('typeDescriptions', {}).get('typeString', ''):
