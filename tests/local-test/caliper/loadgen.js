...
const util = require(`../../../../${caliperName}/src/comm/util.js`);
const RateInterface = require(`../../../../${caliperName}/src/comm/rate-control/rateInterface.js`);
...
const logger = util.getLogger('loadgen');
let mutex = new Mutex();
...
let rwSetDecoderFunction = rewire(`../../../../${caliperName}/node_modules/fabric-client/lib/BlockDecoder.js`).__get__('decodeProposalResponsePayload');
...
module.exports.init = async (bc, ctx, args) => {
    ...
    let workloadFilePath = path.join(workloadDir, workloadFile);
    let workloadModule = require(workloadFilePath);
    workload = workloadModule.buildWorkload(clients, addresses, ctx.evmContractDescriptors);
    logger.info(`Loaded workload with ${workload.length} TXs, from file: ${workloadFilePath}`);
...
module.exports.run = async () => {
    ...
    try {
        ...
        else 
        {
            ...
            await setWriteCpu(id,cpuStart);
            await setWriteMem(id,memStart,memEnds);
            ...
            if (currentTx.args) {
                for (let i = 0; i < currentTx.args.length; ++i) {
                    ...
                    try {
                        ...
                        logger.info(`Error for writing inputs args ${idx}: ${err}`);
                    }
            ...
            let writeOutput = (decode) => {
                if (result.IsCommitted()) {
                    ...
                    if (decode) {
                        ...
                        logger.debug(`Decoding return value as: ${funcRet}`);
                        res = ethabi.rawDecode(funcRet, result.GetResult());
                    } else {
                        logger.debug(`Return value: ${result.GetResult()}`);
            ...
            let writeEntries = (decode) => {
                for (let entry of result.GetCustomData().entries()) {
                    if (entry[0].includes('result') && !entry[0].includes('error')) {
                        let res;
                        if (decode) {
                            let funcRet = context.evmContractDescriptors[contract].methodSignatures[currentTx.function].returnTypes;
                            logger.debug(`Decoding ${entry[0]} as: ${funcRet}`);
            ...
            try {
                ...
                logger.debug(`ERROR for TX ${idx}: ${err}`);
            }
        ...
        //logger.info(`Error invoking ${idx}: ${err}`);
        release();
        return [];
    }
};
...
module.exports.end = async () => {
    await util.sleep(1000); // wait for flushing every tx data
    writeStream.end();
};
...
async function checkMem() {
   ...
   async function setWriteCpu(id,cpuUsage) {
    await cpuUsage.then( j=> { writeStream.write(`${id};cpu_usage;${j}\n`);})
 
 
     return (true);
    }
 
    async function setWriteMem(id,memStart,memEnd) {
     await memStart.then( j=> { writeStream.write(`${id};mem_start;${j}\n`);})
     await memEnd.then( j=> { writeStream.write(`${id};mem_end;${j}\n`);})
